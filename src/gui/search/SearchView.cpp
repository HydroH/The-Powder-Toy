	...
	publishButtonShown(false)
{
	...
	pageLabel = new ui::Label(ui::Point(0, WINDOWH-18), ui::Point(30, 16), TEXT_GUI_SEARCH_LABEL_PAGE1); //page [TEXTBOX] of y
	...
	removeSelected = new ui::Button(ui::Point(((WINDOWW-415)/2), WINDOWH-18), ui::Point(100, 16), TEXT_GUI_SEARCH_EDIT_BTN_DELETE);
	removeSelected->Visible = false;
	removeSelected->SetActionCallback(new RemoveSelectedAction(this));
	AddComponent(removeSelected);

	unpublishSelected = new ui::Button(ui::Point(((WINDOWW-415)/2)+105, WINDOWH-18), ui::Point(100, 16), TEXT_GUI_SEARCH_EDIT_BTN_UNPUB);
	...
	favouriteSelected = new ui::Button(ui::Point(((WINDOWW-415)/2)+210, WINDOWH-18), ui::Point(100, 16), TEXT_GUI_SEARCH_EDIT_BTN_FAV);
	...
	clearSelection = new ui::Button(ui::Point(((WINDOWW-415)/2)+315, WINDOWH-18), ui::Point(100, 16), TEXT_GUI_SEARCH_EDIT_BTN_CLEAR);
...
void SearchView::NotifySortChanged(SearchModel * sender)
{
	if(sender->GetSort() == "best")
	{
		...
		sortButton->SetText(TEXT_GUI_SEARCH_SEARCH_BTN_SORT_VOTE);
		sortButton->SetIcon(IconVoteSort);
	}
	else
	{
		sortButton->SetToggleState(true);
		sortButton->SetText(TEXT_GUI_SEARCH_SEARCH_BTN_SORT_DATE);
...
void SearchView::NotifySaveListChanged(SearchModel * sender)
{
	...
	if(sender->GetShowFavourite())
		favouriteSelected->SetText(TEXT_GUI_SEARCH_EDIT_BTN_UNFAV);
	else
		favouriteSelected->SetText(TEXT_GUI_SEARCH_EDIT_BTN_FAV);
	...
	if (!saves.size())
	{
		...
		if (!errorLabel)
		{
			errorLabel = new ui::Label(ui::Point((WINDOWW/2)-100, (WINDOWH/2)-6), ui::Point(200, 12), TEXT_GUI_SEARCH_ERR_TITLE);
			AddComponent(errorLabel);
		}
		if (!sender->GetSavesLoaded())
		{
			errorLabel->SetText(TEXT_GUI_SEARCH_ERR_LOAD_TITLE);
...
void SearchView::NotifySelectedChanged(SearchModel * sender)
{
	...
	if (selected.size())
	{
		...
		{
			unpublishSelected->SetText(TEXT_GUI_SEARCH_EDIT_BTN_PUB);
			publishButtonShown = true;
		}
		else
		{
			unpublishSelected->SetText(TEXT_GUI_SEARCH_EDIT_BTN_UNPUB);
			publishButtonShown = false;
		}
	}
